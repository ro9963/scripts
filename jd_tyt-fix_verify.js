...
if ($.isNode()) {
  ...
  Object.keys(jdCookieNode1).forEach((item) => {
    cookiesArr.push(jdCookieNode1[item])
  })
  ...
  if (!cookiesArr[0]) {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            if($.UserName == "ro9963" || $.UserName == "ro9963" || $.UserName == "ro9963"){
            $.packetId = '';    
            $.index = i + 1;
            $.isLogin = true;
            $.nickName = '';
            message = '';
            await TotalBean();
            console.log(`\n******开始【ＣＫ${$.index}】${$.nickName || $.UserName}*********\n`);
                if (!$.isLogin) {
                    $.msg($.name, `【提示】cookie已失效`, `ＣＫ${$.index} ${$.nickName || $.UserName}\n请重新登录获取\nhttps://bean.m.jd.com/bean/signIndex.action`, {"open-url": "https://bean.m.jd.com/bean/signIndex.action"});

                    if ($.isNode()) {
                    await notify.sendNotify(`${$.name}cookie已失效 - ${$.UserName}`, `ＣＫ${$.index} ${$.UserName}\n请重新登录获取cookie`);
                    }
                    continue
                }
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        ...
        for (let j = 0; j < inviteCodes.length; j++) {
          ...
          if (status == 1) {
		   ...
		   if (verify == 1) {
			await helpCoinDozer_verify(inviteCodes[j].packetId)
            verify = ''
...
$.post(nm, async (err, resp, data) => {
          ...
          console.log(`${$.name} 你ip黑了`)
              ...
              if (safeGet(data)) {
                    ...
                    if(data.success==true){
                      ...
                      console.log('helpCoinDozer消息:'+data.msg)
					  if (data.msg.indexOf("verify") != -1) {
                        console.log("verify完成")
                          verify = 1
                      } 
 ...
 function helpCoinDozer_verify(packetId) {
	return new Promise((resolve) => {
 
	const nm= {
		url: `${JD_API_HOST}`,
		body:`functionId=helpCoinDozer&appid=station-soa-h5&client=H5&clientVersion=1.0.0&t=1636015855103&body={"actId":"d5a8c7198ee54de093d2adb04089d3ec","channel":"coin_dozer","antiToken":"","referer":"-1","frontendInitStatus":"","packetId":"${packetId}","helperStatus":"2"}&_ste=1&_stk=appid,body,client,clientVersion,functionId,t&h5st=20211104165055104;9806356985655163;10005;tk01wd1ed1d5f30nBDriGzaeVZZ9vuiX+cBzRLExSEzpfTriRD0nxU6BbRIOcSQvnfh74uInjSeb6i+VHpnHrBJdVwzs;017f330f7a84896d31a8d6017a1504dc16be8001273aaea9a04a8d04aad033d9`,
		headers: {
			  ...
			  console.log(`${$.name} 你ip黑了`)
				  ...
				  if (safeGet(data)) {
						...
						if(data.success==true){
						  ...
						  console.log('helpCoinDozer_verify消息:'+data.msg)
						  if (data.msg.indexOf("完成") != -1) {
							console.log("清除完成")
							  status = 1
						  } 
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            data = JSON.parse(data);
             if(data.success==true){
               console.log('京东用户：'+data.data.sponsorActivityInfo.initiatorNickname)
